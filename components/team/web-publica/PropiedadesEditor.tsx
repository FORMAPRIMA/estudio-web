...
async function uploadImage(file: File): Promise<{ url: string } | { error: string }> {
  ...
  const { data, error } = await supabase.storage.from(BUCKET).upload(`propiedades/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`, file, { cacheControl: '31536000', upsert: false })
