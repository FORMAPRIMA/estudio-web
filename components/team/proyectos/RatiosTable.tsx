      ...
      <div className="space-y-8 max-w-2xl">
        {fasesBySection.map(group => (
          <div key={group.seccion}>
            ...
            <div className="ratios-group border border-ink/10">
              {/* Header */}
              <div className="ratios-row-header grid grid-cols-[2rem_1fr_10rem_6rem] gap-4 px-4 py-2 border-b border-ink/8 bg-ink/[0.015]">
              ...
              {group.items.map((fase, i) => (
                <div
                  key={fase.id}
                  className={`ratios-row grid grid-cols-[2rem_1fr_10rem_6rem] gap-4 items-center px-4 py-3 ${
                    i < group.items.length - 1 ? 'border-b border-ink/6' : ''
                  }`}
                >
                  <span className="ratios-col-num text-[10px] tracking-widest uppercase font-light text-meta">
                    F{fase.numero}
                  </span>
                  <span className="ratios-col-label text-sm font-light text-ink">{fase.label}</span>
                  <div className="ratios-col-input flex justify-end">
                    <input
                      type="number"
                      step="0.0001"
                      min="0"
                  ...
                  <div className="ratios-col-action flex items-center gap-2">
                    <button
