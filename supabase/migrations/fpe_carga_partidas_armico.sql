-- ============================================================================
-- fpe_carga_partidas_armico.sql
--
-- Carga del template FP Execution con partidas reales del presupuesto
-- Armico (Francisca Moreno 6).
--
-- Cambios estructurales:
--   1. Nueva disciplina: Solador y alicatador
--   2. ALBAÑILERÍA (30): nueva unidad "Enlucidos y revoques"
--   3. Reorganización de capítulos a patrón de 4 unidades (subcontratista):
--      60, 80, 81, 82, 84, 90, 100, 110, 120, 130
--   4. ACABADOS (145): 4 unidades nuevas
--
-- Datos:
--   - Borra todos los template_line_items existentes (eran placeholders)
--   - Borra los project_units/project_line_items/bids (eran datos de prueba)
--   - Carga 111 partidas del presupuesto Armico con sus disciplinas y
--     fases de ejecución vinculadas
-- ============================================================================
...
DELETE FROM public.fpe_bid_phase_durations;
DELETE FROM public.fpe_bid_line_items;
DELETE FROM public.fpe_bids;
DELETE FROM public.fpe_project_line_items;
DELETE FROM public.fpe_project_units;
DELETE FROM public.proyecto_memoria_items;   -- snapshots por proyecto (data de prueba)
DELETE FROM public.warehouse_items;          -- catálogo de prueba (1 item)
DELETE FROM public.fpe_template_line_items;

DELETE FROM public.fpe_template_units
WHERE chapter_id IN (
  SELECT id FROM public.fpe_template_chapters
  WHERE orden IN (60, 80, 90, 100, 110, 120, 130)
);
...
INSERT INTO public.fpe_disciplines (nombre, descripcion, color, orden, activo)
VALUES (
  'Solador y alicatador',
  'Instalación de pavimentos y revestimientos cerámicos, porcelánicos, gres y similares.',
  '#9C7C5E',
  90,
  true
);
...
-- ALBAÑILERÍA (30): "Enlucidos y revoques"
INSERT INTO public.fpe_template_units (chapter_id, nombre, orden)
SELECT id, 'Enlucidos y revoques', 80
FROM public.fpe_template_chapters WHERE orden = 30;

-- FONTANERÍA Y SANEAMIENTO (60)
INSERT INTO public.fpe_template_units (chapter_id, nombre, orden)
SELECT (SELECT id FROM public.fpe_template_chapters WHERE orden = 60), v.nombre, v.orden
FROM (VALUES
  ('Equipos', 10),
  ('Red de fontanería', 20),
  ('Red de saneamiento', 30),
  ('Mano de obra e instalación', 40)
) AS v(nombre, orden);

-- CLIMATIZACIÓN A/C (80)
INSERT INTO public.fpe_template_units (chapter_id, nombre, orden)
SELECT (SELECT id FROM public.fpe_template_chapters WHERE orden = 80), v.nombre, v.orden
FROM (VALUES
  ('Equipo exterior', 10),
  ('Equipo interior', 20),
  ('Material y distribución', 30),
  ('Mano de obra e instalación', 40)
) AS v(nombre, orden);

-- CALEFACCIÓN INDIVIDUAL (81)
INSERT INTO public.fpe_template_units (chapter_id, nombre, orden)
SELECT (SELECT id FROM public.fpe_template_chapters WHERE orden = 81), v.nombre, v.orden
FROM (VALUES
  ('Equipo de generación', 10),
  ('Equipo emisor', 20),
  ('Material y distribución', 30),
  ('Mano de obra e instalación', 40)
) AS v(nombre, orden);

-- CALEFACCIÓN CENTRAL (82)
INSERT INTO public.fpe_template_units (chapter_id, nombre, orden)
SELECT (SELECT id FROM public.fpe_template_chapters WHERE orden = 82), v.nombre, v.orden
FROM (VALUES
  ('Equipo emisor', 10),
  ('Material y distribución', 20),
  ('Mano de obra e instalación', 30)
) AS v(nombre, orden);

-- SUELO RADIANTE (84)
INSERT INTO public.fpe_template_units (chapter_id, nombre, orden)
SELECT (SELECT id FROM public.fpe_template_chapters WHERE orden = 84), v.nombre, v.orden
FROM (VALUES
  ('Equipo de generación', 10),
  ('Sistema emisor', 20),
  ('Material y distribución', 30),
...
-- VENTILACIÓN (90)
INSERT INTO public.fpe_template_units (chapter_id, nombre, orden)
SELECT (SELECT id FROM public.fpe_template_chapters WHERE orden = 90), v.nombre, v.orden
FROM (VALUES
  ('Equipos', 10),
  ('Conductos y red', 20),
...
-- ELECTRICIDAD (100)
INSERT INTO public.fpe_template_units (chapter_id, nombre, orden)
SELECT (SELECT id FROM public.fpe_template_chapters WHERE orden = 100), v.nombre, v.orden
FROM (VALUES
  ('Cuadro y protecciones', 10),
  ('Distribución y cableado', 20),
  ('Iluminación', 30),
  ('Mano de obra e instalación', 40)
) AS v(nombre, orden);
...
INSERT INTO public.fpe_template_units (chapter_id, nombre, orden)
SELECT (SELECT id FROM public.fpe_template_chapters WHERE orden = 110), v.nombre, v.orden
FROM (VALUES
  ('Equipos', 10),
  ('Red y cableado', 20),
  ('Tomas y dispositivos terminales', 30),
  ('Mano de obra e instalación', 40)
) AS v(nombre, orden);

-- DOMÓTICA (120)
INSERT INTO public.fpe_template_units (chapter_id, nombre, orden)
SELECT (SELECT id FROM public.fpe_template_chapters WHERE orden = 120), v.nombre, v.orden
FROM (VALUES
  ('Equipos centrales', 10),
  ('Material y cableado', 20),
  ('Sensores e interfaces de usuario', 30),
  ('Mano de obra e instalación', 40)
) AS v(nombre, orden);

-- SONIDO (130)
INSERT INTO public.fpe_template_units (chapter_id, nombre, orden)
SELECT (SELECT id FROM public.fpe_template_chapters WHERE orden = 130), v.nombre, v.orden
FROM (VALUES
  ('Equipos', 10),
  ('Material y cableado', 20),
  ('Altavoces y elementos visibles', 30),
  ('Mano de obra e instalación', 40)
) AS v(nombre, orden);

-- ACABADOS (145)
INSERT INTO public.fpe_template_units (chapter_id, nombre, orden)
SELECT (SELECT id FROM public.fpe_template_chapters WHERE orden = 145), v.nombre, v.orden
FROM (VALUES
  ('Pavimentos cerámicos y porcelánicos', 10),
  ('Revestimientos cerámicos y porcelánicos', 20),
  ('Pavimentos de madera y laminados', 30),
  ('Pavimentos continuos', 40)
) AS v(nombre, orden);

-- ---------------------------------------------------------------------------
-- 4. Carga de partidas — todo en una sola consulta con CTEs encadenados
--    para evitar problemas con tablas intermedias en el SQL editor.
-- ---------------------------------------------------------------------------
