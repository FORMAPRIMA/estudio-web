...
export default async function QuinielaPublicRoute() {
  ...
  if ('error' in data) {
    if (data.error === 'SIN_SESION') {
      ...
      return <QuinielaGate monto={monto} numJugadores={count ?? 0} />
    }
    return (
      <div style={{ minHeight: '100vh', background: '#F8F7F4', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <p style={{ fontSize: 13, color: '#1A1A1A99' }}>La porra no está disponible ahora mismo. Vuelve a intentarlo en un rato.</p>
      </div>
    )
  }
