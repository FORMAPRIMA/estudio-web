...
import { SECCION_CONSTRUCTORA, esFacturaNoCliente } from '@/lib/finanzas/costs'
...
export default async function Page({
  ...
  if (searchParams.from) {
    ...
    if (f) {
      ...
      // ── Facturas a PROVEEDOR: nunca se facturan al cliente ───────────────────
      // Lo decide el destinatario de la factura, no solo su sección:
      //  - "Margen prorrateado de obra" → constructora del proyecto (constructor_id)
      //  - Cualquier factura con proveedor_id → ese proveedor. Es el caso de los
      //    rappels y descuentos de proveedores de mobiliario, que están en
      //    "Compra de mobiliario", una sección donde los suplidos sí van al cliente.
      const seccionF = f.seccion as string | null
      const provIdF  = (f as Record<string, unknown>).proveedor_id as string | null
      if (esFacturaNoCliente({ seccion: seccionF, proveedorId: provIdF })) {
        ...
        if (provId) {
          const { data: prov } = await admin
            .from('proveedores')
            .select('id, nombre, razon_social, nif_cif, direccion_fiscal, direccion, contacto_nombre, condiciones_pago')
            .eq('id', provId)
            .maybeSingle()
          if (prov) {
            prefill.proveedorId      = prov.id as string
            prefill.clienteEmpresa   = (prov.razon_social as string | null) ?? prov.nombre
            prefill.clienteContacto  = (prov.contacto_nombre as string | null) ?? ''
