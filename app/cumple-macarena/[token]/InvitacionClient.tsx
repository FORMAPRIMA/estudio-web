...
const MENU_OPTIONS = [
  { id: 'menu_a', emoji: '🍗', label: 'Menú A', desc: 'Placeholder opción A' },
  { id: 'menu_b', emoji: '🍝', label: 'Menú B', desc: 'Placeholder opción B' },
  { id: 'menu_c', emoji: '🥗', label: 'Menú C', desc: 'Placeholder opción C' },
]

type RsvpStep = 'idle' | 'selecting_menu' | 'submitting' | 'done_yes' | 'done_no'

type Props = {
  token: string
  nombre: string
  existingRsvp: { asiste: boolean; menu_opcion: string | null } | null
}
...
.display{font-family:'Bowlby One',sans-serif;font-weight:400;letter-spacing:.5px}
...
.guest-pill{position:relative;z-index:3;margin-top:14px;display:inline-flex;align-items:center;gap:8px;padding:8px 16px 8px 10px;background:rgba(255,255,255,.12);backdrop-filter:blur(8px);border:2px dashed rgba(255,217,59,.7);border-radius:999px;color:#fff;font-family:'Caveat',cursive;font-size:22px;line-height:1;animation:guestIn 1s .2s cubic-bezier(.34,1.56,.64,1) backwards;max-width:92%}
.guest-pill .gift{width:28px;height:28px;border-radius:50%;background:var(--yellow);display:inline-flex;align-items:center;justify-content:center;border:2px solid var(--ink);box-shadow:0 2px 0 var(--yellow-deep);color:var(--denim-deep);font-family:'Bowlby One';font-size:14px}
.guest-pill .gn{font-weight:700;color:var(--yellow);max-width:240px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
@keyframes guestIn{0%{transform:scale(.5) translateY(-10px);opacity:0}100%{transform:scale(1) translateY(0);opacity:1}}
...
.buddy .smile::after{content:"";position:absolute;bottom:-4px;left:6px;right:6px;height:6px;background:#fff;border-radius:0 0 6px 6px}
...
.ticket{background:#fff;border:3px solid var(--ink);border-radius:24px;box-shadow:0 8px 0 var(--denim-deep),0 20px 50px rgba(0,0,0,.15);padding:0;overflow:hidden;position:relative}
...
.jump-card{background:var(--ink);color:#fff;border-radius:28px;overflow:hidden;border:3px solid var(--ink);box-shadow:0 10px 0 var(--denim-deep),0 24px 50px rgba(0,0,0,.2);position:relative}
...
.rsvp > p{font-family:'Caveat';font-size:24px;color:var(--ink);margin-top:6px}
.host-card{margin-top:22px;background:#fff;border:3px solid var(--ink);border-radius:20px;padding:18px 18px 20px;box-shadow:0 6px 0 var(--denim-deep);text-align:left;display:flex;gap:14px;align-items:center}
.host-card .ava{width:54px;height:54px;border-radius:14px;flex-shrink:0;background:var(--yellow);border:2px solid var(--ink);box-shadow:0 3px 0 var(--yellow-deep);display:flex;align-items:center;justify-content:center;font-family:'Bowlby One';font-size:22px;color:var(--denim-deep)}
.host-card .info{flex:1;min-width:0}
.host-card .lbl{font-size:10px;letter-spacing:.2em;text-transform:uppercase;color:var(--denim);font-weight:600}
.host-card .name2{font-family:'Bowlby One';font-size:18px;color:var(--ink);margin-top:2px}
.host-card .phone{font-size:13px;color:rgba(14,26,51,.7);margin-top:2px;font-weight:500}
.cta-row{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:18px}
.btn{display:flex;align-items:center;justify-content:center;gap:8px;padding:16px 12px;border-radius:16px;font-family:'Fredoka';font-weight:700;font-size:15px;border:3px solid var(--ink);text-decoration:none;cursor:pointer;transition:transform .15s ease,box-shadow .15s ease;position:relative;background:none}
.btn:active{transform:translateY(2px)}
.btn.yes{background:var(--grass);color:#fff;box-shadow:0 5px 0 var(--grass-deep)}
.btn.yes:hover{transform:translateY(-2px);box-shadow:0 7px 0 var(--grass-deep)}
.btn.maybe{background:#fff;color:var(--denim-deep);box-shadow:0 5px 0 var(--denim-deep)}
.btn.maybe:hover{transform:translateY(-2px);box-shadow:0 7px 0 var(--denim-deep)}
.deadline{margin-top:14px;font-size:12px;color:var(--denim-deep);font-weight:600;letter-spacing:.05em}
.deadline strong{color:var(--red)}

/* MENU SELECTION */
.menu-section{margin-top:20px;text-align:left}
.menu-section .menu-title{font-family:'Bowlby One';font-size:20px;color:var(--denim-deep);text-align:center;margin-bottom:14px}
.menu-cards{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:16px}
.menu-card{padding:14px 8px;border-radius:16px;border:3px solid transparent;background:#fff;text-align:center;cursor:pointer;transition:all .15s ease;box-shadow:0 4px 0 rgba(0,0,0,.08)}
.menu-card:hover{transform:translateY(-2px)}
.menu-card.selected{border-color:var(--denim);background:var(--denim-soft);color:#fff;box-shadow:0 4px 0 var(--denim-deep)}
.menu-card .me{font-size:28px;display:block;margin-bottom:6px}
.menu-card .ml{font-family:'Bowlby One';font-size:13px;color:var(--ink)}
.menu-card.selected .ml{color:#fff}
.menu-card .md{font-size:10px;color:rgba(14,26,51,.6);margin-top:2px;font-weight:500}
.menu-card.selected .md{color:rgba(255,255,255,.8)}
.btn-confirm{width:100%;padding:18px;border-radius:16px;border:3px solid var(--ink);background:var(--yellow);color:var(--denim-deep);font-family:'Bowlby One';font-size:20px;cursor:pointer;box-shadow:0 5px 0 var(--yellow-deep);transition:all .15s ease}
.btn-confirm:hover:not(:disabled){transform:translateY(-2px);box-shadow:0 7px 0 var(--yellow-deep)}
...
.rsvp-done .menu-badge{display:inline-flex;align-items:center;gap:6px;margin-top:10px;background:var(--yellow);padding:6px 14px;border-radius:999px;border:2px solid var(--ink);font-weight:600;font-size:13px;color:var(--denim-deep)}

/* FOOTER */
.foot{background:var(--ink);color:#fff;padding:24px 22px 30px;text-align:center;position:relative}
...
.music-btn:active{transform:translateY(2px);box-shadow:0 2px 0 var(--yellow-deep)}
...
.music-btn:not(.on) .bars{display:none}
...
.music-btn .bars i:nth-child(1){height:60%;animation-delay:0s}
...
function BuddyB1() {
  return (
    <div className="buddy b1">
      ...
      <div className="arm l"><div className="hand" /></div>
      <div className="arm r"><div className="hand" /></div>
      <div className="leg l"><div className="shoe" /></div>
      <div className="leg r"><div className="shoe" /></div>
    </div>
  )
}
...
export default function InvitacionClient({ token, nombre, existingRsvp }: Props) {
  const initialStep: RsvpStep = existingRsvp
    ? (existingRsvp.asiste ? 'done_yes' : 'done_no')
    : 'idle'

  const [rsvpStep, setRsvpStep] = useState<RsvpStep>(initialStep)
  const [selectedMenu, setSelectedMenu] = useState<string>(existingRsvp?.menu_opcion ?? '')
  const [musicOn, setMusicOn] = useState(false)
  const [hintGone, setHintGone] = useState(false)

  const audioRef = useRef<{ ctx: AudioContext; master: GainNode; scheduler: number | null } | null>(null)
  const melTimeRef = useRef(0)
  const melStepRef = useRef(0)
  ...
  // Countdown
  useEffect(() => {
    const target = new Date('2026-06-13T17:00:00+02:00').getTime()
  ...
  const celebrate = useCallback((label: string) => {
    ...
    Object.assign(t.style, {
      ...
      fontFamily: "'Bowlby One',sans-serif", fontSize: '56px',
      color: '#FFD93B', WebkitTextStroke: '3px #13316E',
      textShadow: '0 6px 0 #13316E',
  ...
  const startMusic = useCallback(() => {
    ...
    melTimeRef.current = nextTimeRef.current
    stepRef.current = 0; melStepRef.current = 0

    function tick() {
      const lookahead = ctx.currentTime + 0.5
      while (nextTimeRef.current < lookahead) {
        const bi = stepRef.current % 16
        const b = bass[bi]
      ...
      while (melTimeRef.current < lookahead) {
  ...
  // RSVP handlers
  const handleYes = () => setRsvpStep('selecting_menu')

  const handleNo = async () => {
    setRsvpStep('submitting')
    try {
      await submitRsvp(token, false, null, null)
      celebrate('¡Te echaremos de menos! 💛')
      setRsvpStep('done_no')
    } catch {
      setRsvpStep('idle')
    }
  ...
  const handleConfirmMenu = async () => {
    ...
    setRsvpStep('submitting')
    try {
      await submitRsvp(token, true, selectedMenu, null)
      celebrate('¡YUPI! 🎉')
      setRsvpStep('done_yes')
    } catch {
      setRsvpStep('selecting_menu')
    }
  }

  const selectedMenuLabel = MENU_OPTIONS.find(m => m.id === selectedMenu)

  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: CSS }} />

      <div className="intro" id="cumple-intro">
        <div className="big">¡Hey!</div>
      </div>

      <button className={`music-btn${musicOn ? ' on' : ''}`} onClick={toggleMusic} aria-label="Activar música">
        <svg className="ico-mute" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round">
      ...
      <main>
        ...
        <section className="hero">
          ...
          <div className="guest-pill" style={{ display: 'inline-flex' }}>
            <span className="gift">★</span>
            <span>Hola, <span className="gn">{nombre.charAt(0).toUpperCase() + nombre.slice(1)}</span> — ¡estás invitada!</span>
          </div>
          ...
          <div className="ticket reveal">
            ...
            <div className="ticket-body">
              ...
              <div className="ticket-row">
                ...
                <div><div className="lbl">Hora</div><div className="val">17:00 — 19:30 h</div><div className="sub">Llega 10 min antes para el check-in</div></div>
              </div>
              <div className="ticket-row">
                <div className="ico">
                ...
                <div><div className="lbl">Lugar</div><div className="val">Parque de trampolines</div><div className="sub">Las Rejas, Madrid · Plazas limitadas</div></div>
              </div>
              <div className="ticket-row">
                <div className="ico">
            ...
            <div className="ticket-foot">
              ...
              <div className="num" style={{ fontSize: 18 }}>★★★<small>vip pass</small></div>
            </div>
          </div>
        </section>
          ...
          <div className="jump-card reveal">
            ...
            <div className="jump-info">
              ...
              <div className="place">
                ...
                Parque indoor · Las Rejas (Madrid)
              </div>
        ...
        <section className="rsvp">
          ...
          <p className="reveal">Confírmanos plis 🎈</p>

          <div className="host-card reveal">
            <div className="ava">M</div>
            <div className="info">
              <div className="lbl">Anfitriones</div>
              <div className="name2">Mamá &amp; Papá de Maca</div>
              <div className="phone">Avísanos por WhatsApp</div>
            </div>
          </div>
          ...
          {rsvpStep === 'idle' && (
            <div className="cta-row reveal">
              ...
              <button className="btn maybe" onClick={handleNo}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M9 9a3 3 0 1 1 4.5 2.6c-.9.5-1.5 1.1-1.5 2.4M12 17.5h.01" /></svg>
                No puedo ir
              </button>
            </div>
          )}

          {rsvpStep === 'selecting_menu' && (
            <div className="menu-section reveal">
              <div className="menu-title">¿Qué prefieres comer? 🍽️</div>
              <div className="menu-cards">
                {MENU_OPTIONS.map(opt => (
                  <div
                    key={opt.id}
                    className={`menu-card${selectedMenu === opt.id ? ' selected' : ''}`}
                    onClick={() => setSelectedMenu(opt.id)}
                  >
                    <span className="me">{opt.emoji}</span>
                    <div className="ml">{opt.label}</div>
                    <div className="md">{opt.desc}</div>
                  </div>
                ))}
              </div>
              <button
          ...
          {rsvpStep === 'submitting' && (
            <div className="rsvp-done reveal">
              ...
              <div className="done-title">Guardando…</div>
            </div>
          )}
          ...
          {rsvpStep === 'done_yes' && (
            <div className="rsvp-done reveal">
              <span className="big-emoji">🎉</span>
              <div className="done-title">¡Cuenta confirmada!</div>
              <div className="done-sub">Te esperamos el 13 de junio 💛</div>
              {(selectedMenuLabel ?? existingRsvp?.menu_opcion) && (
                <div className="menu-badge">
                  {selectedMenuLabel?.emoji ?? ''} {selectedMenuLabel?.label ?? existingRsvp?.menu_opcion}
                </div>
              )}
            </div>
          )}
          ...
          {rsvpStep === 'done_no' && (
            <div className="rsvp-done reveal">
              ...
              <div className="done-sub">Te echaremos de menos, ¡gracias por avisarnos!</div>
            </div>
          )}

          <div className="deadline reveal">Confirma antes del <strong>6 de junio</strong></div>
        </section>
