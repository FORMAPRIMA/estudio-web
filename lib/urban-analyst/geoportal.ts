...
import { toEsriRings, esriRingsToGeoJSON } from './geometry'
import type { GeoJSONGeometry, HitCategoria } from './types'
...
export const GEOPORTAL_SERVICES: ServiceConfig[] = [
  ...
  { service: 'DESARROLLO_URBANO_ACTUALIZADO/AMBITOS_PLANEAMIENTO_URBANISTICO', categoria: 'ambito',        label: 'Ámbitos de planeamiento (APE/APR/API...)' },
  { service: 'DESARROLLO_URBANO_ACTUALIZADO/PLANEAMIENTO_URBANISTICO',        categoria: 'planeamiento',  label: 'Modificaciones y desarrollos del PGOUM' },
  { service: 'DESARROLLO_URBANO_ACTUALIZADO/USOS_SUELO',                      categoria: 'uso_suelo',     label: 'Usos del suelo' },
  { service: 'DESARROLLO_URBANO_ACTUALIZADO/BIC',                             categoria: 'bic',           label: 'Bienes de Interés Cultural' },
...
export async function queryAllServices(geometry: GeoJSONGeometry): Promise<{
  ...
  const settled = await Promise.allSettled(
    GEOPORTAL_SERVICES.map((cfg) => queryService(cfg, geometry))
  )
